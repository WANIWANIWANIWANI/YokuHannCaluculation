...
{xc},{yc}
{r}
""".format(xc=circle.O.x+O.x,yc=circle.O.y+O.y,r=circle.r))
	if WriteCenter:
...
""".format(ax=stringer.A.x+O.x, ay=stringer.A.y+O.y,bx=stringer.B.x+O.x, by=stringer.B.y+O.y,cx=stringer.C.x+O.x, cy=stringer.C.y+O.y, dx=stringer.D.x+O.x, dy=stringer.D.y+O.y))
...
def define_Oa(EndFoilPs, RootFoilPs, da, h, EndPipeO, RootPipeO, sharpness=0.1, StartO = vector(0,0)):
	"""
	...
	for i in range(2):
		...
		if relation(FoilPs[1], r, ret[0]+Os[1]) == "Separated":	#ret[0]がRootでも中に入っていたら
			return ret[0]
	return ret[1]
...
f_uEnd = inter(EndFoilDataU_x[::-1]*EndChord*cos(sweep), EndFoilDataU_y[::-1]*EndChord)
f_dEnd = inter(EndFoilDataD_x*EndChord*cos(sweep), EndFoilDataD_y*EndChord)
f_uRoot = inter(RootFoilDataU_x[::-1]*RootChord*cos(sweep), RootFoilDataU_y[::-1]*RootChord)
...
RootFoilPs = [vector(P.x*RootChord*cos(sweep), P.y*RootChord) for P in RootFoilData]
#パイプの中心
EndPipeO = vector(EndR*EndChord/100, f_uEnd(0.25*EndChord)/2 + f_dEnd(0.25*EndChord)/2)
RootPipeO = vector(RootR*RootChord/100, f_uRoot(0.25*RootChord)/2 + f_dRoot(0.25*RootChord)/2)
#アセンブリ棒の準備
Oa = define_Oa(EndFoilPs, RootFoilPs, da, h+t, EndPipeO, RootPipeO, StartO=vector(0.6*EndChord, f_uEnd(0.6*EndChord)/2 + f_dEnd(0.6*EndChord)/2)-EndPipeO)
...
for k in range(1,n+1):#range(1,n+1):				 	#根から k 枚目のリブ
	...
	#プランクの点のリストの出力
	PlankPs = offset([FoilU[i] for i in range(len(FoilU)-2) if FoilU[i+2].x <= x_plank_u] + [FoilU[-2],FoilD[0],FoilD[1]] + [FoilD[i] for i in range(2,len(FoilD)) if FoilD[i-2].x <= x_plank_d], tp, 0)
	...
	stringerU1 =  [PlankPsU[i] for i in range(1,len(PlankPsU)) if PlankPsU[i-1].x <= x_stringer_u1][-2:]
	stringerU1ToVec = stringer(div_P(stringerU1[0], stringerU1[1], x_stringer_u1, 0), stringerU1[0], e)

	stringerU2= [PlankPsU[i] for i in range(1,len(PlankPsU)) if PlankPsU[i-1].x <= x_stringer_u2][-2:]
	stringerU2ToVec = stringer(div_P(stringerU2[0], stringerU2[1], x_stringer_u2, 0), stringerU2[0], e)

	stringerU3= [PlankPsU[i] for i in range(1,len(PlankPsU)) if PlankPsU[i-1].x <= x_stringer_u3][-2:]
	stringerU3ToVec = stringer(div_P(stringerU3[0], stringerU3[1], x_stringer_u3, 0), stringerU3[0], e)
    
	stringerD1= [PlankPsD[i] for i in range(1,len(PlankPsD)) if PlankPsD[i-1].x <= x_stringer_D1][-2:]
	stringerD1ToVec = stringer(div_P(stringerD1[0], stringerD1[1], x_stringer_D1, 0), stringerD1[0], e, R=True)

	stringerD2= [PlankPsD[i] for i in range(1,len(PlankPsD)) if PlankPsD[i-1].x <= x_stringer_D2][-2:]
	stringerD2ToVec = stringer(div_P(stringerD2[0], stringerD2[1], x_stringer_D2, 0), stringerD2[0], e, R=True)

	stringerD3= [PlankPsD[i] for i in range(1,len(PlankPsD)) if PlankPsD[i-1].x <= x_stringer_D3][-2:]
	stringerD3ToVec = stringer(div_P(stringerD3[0], stringerD3[1], x_stringer_D3, 0), stringerD3[0], e, R=True)
	...
	if use_l:
		...
		LeadD = [P + vector(lo,0) for P in FoilD[1:] if P.x < c*cos(sweep)*rpd/100]
		...
		LeadPs = LeadU + LeadD
		#前縁材端の線の端点
		LeadEndP2U = LeadPs[0] + ((vector(LeadPs[0].x+tp, f_pu(LeadPs[0].x+tp)) - LeadPs[0])/abs(vector(LeadPs[0].x+tp, f_pu(LeadPs[0].x+tp)) - LeadPs[0])).i * tp
		LeadEndP2D = LeadPs[-1] + ((vector(LeadPs[-1].x-tp, f_pd(LeadPs[-1].x-tp)) - LeadPs[-1])/abs(vector(LeadPs[-1].x-tp, f_pd(LeadPs[-1].x-tp)) - LeadPs[-1])).i * tp
	...
	y_tl = [f_d(x_tl[0])+w_tri+r_tri, f_d(x_tl[1])+w_tri+r_tri, f_u(x_tl[1])-w_tri-r_tri]
	x_tl = x_tl + [x_tl[1] - (y_tl[2]-y_tl[1])*tan((2*r_tri+w_tri)/(y_tl[2]-y_tl[1]))]
	y_tl[2] = f_u(x_tl[2])-w_tri-r_tri
	tri_lead_circles = [circle(r_tri, vector(x_tl[i],y_tl[i])) for i in range(3)]
	tri_lead_lines = [offset([tri_lead_circles[i-1].O, tri_lead_circles[i].O], r_tri, 1, 1) for i in range(3)]
	#前縁側 右上、左上、下の順
	x_tt = [x_tri_trail, x_tl[1]]
	y_tt = [f_u(x_tt[0])-w_tri-r_tri, f_u(x_tt[1])-w_tri-r_tri, f_u((x_tri_lead+x_tri_trail)/2)-w_tri-r_tri]
	x_tt = x_tt + [x_tt[1] + (y_tl[2]-y_tl[1])*tan((2*r_tri+w_tri)/(y_tl[2]-y_tl[1]))]
	y_tt[2] = f_d(x_tt[2])+w_tri+r_tri
	tri_trail_circles = [circle(r_tri, vector(x_tt[i],y_tt[i])) for i in range(3)]
	tri_trail_lines = [offset([tri_trail_circles[i-1].O, tri_trail_circles[i].O], r_tri, 1, 1) for i in range(3)]

	#丸肉抜き出力 前縁から
	#最前縁の丸の中心の座標
	x_cir = Pipe.C.x + (d+dd)/2
	x_cir += (f_u(x_cir)-f_d(x_cir))/2
	light_Cs = [circle((f_u(x_cir)-f_d(x_cir))/2 - w_circle, vector(x_cir, f_camber(x_cir)))]
	i = 1
	while True:
	...
	#番号出力 切らないのでピンク
	color(file,255,0,255)
	WriteText(file, vector(c*0.05, O.y+f_camber(c*0.05)), f"{PlaneNumber}-{k}", height=7)

#設定出力 切らないのでピンク
color(file,255,0,255)
WriteText(file, vector(RootChord*1.05, 0), f"""根翼弦 : {RootChord} mm
端翼弦 : {EndChord} mm
根ねじり上げ : {RootDelta} °
端ねじり上げ : {EndDelta} °
根の桁位置 : {RootR} %
