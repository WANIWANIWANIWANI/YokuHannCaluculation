...
for k in range(1, n + 1):  # range(1,n+1):				 	#根から k 枚目のリブ
    ...
    def sannkakuNikunuki(basePointArrayInput, angle):
        ...
        line13 = makeLinearEquation(
            basePoint1_x, basePoint1_y, basePoint3_x, basePoint3_y
        )
        # トラスの外周幅を計算
        gaishuugosa = 0
        diff = 0
        ...
        # 上向きトラスの場合と下向きトラスの場合で距離をはかる直線が異なる
        if len(basePointArrayLists_fixed) % 2 != 0:
            lengthOfTorasu = Calc_distance(
                line23["m"], -1, line23["n"], torasuBefore_x, torasuBefore_y
            )
        elif len(basePointArrayLists_fixed) % 2 == 0:
            lengthOfTorasu = Calc_distance(
                line13["m"], -1, line13["n"], torasuBefore_x, torasuBefore_y
            )
        torasuuNanamezaiGosa = abs(lengthOfTorasu - torasuuAtumi)
        ##設定値からのずれを評価

        gosa = abs(gaishuugosa) * 100 + abs(torasuuNanamezaiGosa)
        if gosa < minimumGosa > 0:
    ...
    torasuuAtumi = 5
    # 各基準点を保持する配列
    basePointArrayLists_fixed = [[(0.05, 0.20), (0.07, 0.20)]]
    # ここに出力を行うトラスの個数を打ち込む
    torasuuumber = 5
    # 各トラスの解析を行う際のループ数を入力
    timesOfKaiseki = 400
    ...
    # 各トラス座標の最適化計算が行われる
    while len(basePointArrayLists_fixed) < torasuuumber:
        print(len(basePointArrayLists_fixed), "を解析中")
                ...
                saerchPointArrayLists.append(
                    [
                        (
                            random.uniform(
                                (
                                    basePointArrayLists_fixed[-1][0][0]
                                    + basePointArrayLists_fixed[-1][1][0]
                                )
                                / 2,
                                (
                                    basePointArrayLists_fixed[-1][0][0]
